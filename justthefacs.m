function justthefacs
   ...
   hColors = [ 0 0 0; 1 0 0; 1 0.5 0; 0 0.7 0; 0 1 1; 0 0 1; 1 0 1; 0.7 0.7 0.7;]; % The colors to cycle through in multi-file histograms
   HistogramBins = logspace(0,5,200); % The usual range of arbitrary units.
   XTicks = [1 10 100 1000 10000 100000];
   XLimits = [1 100000];
 
   %  Begin building the GUI.
   f = figure('Visible','off','MenuBar','none','Resize','off','Name','Just the FACS, man!','Position',[0,0,700,300],'Color',[1 1 1]);
   
   % GUI elements for the list of current files
   hFileLabel = uicontrol('Style','text','String','Files of Interest','Position',[0 285 140 15]);
   hFileList = uicontrol(f, 'Style','listbox','String',{},'Value',1,'Callback',{@ChangeFileSelection_Callback},'Position',[0 215 140 70]);
   hAdd = uicontrol('Style','pushbutton','String','Add','Position',[0,195,70,20],'Callback',{@AddButton_Callback});
   hRemove = uicontrol('Style','pushbutton','String','Remove','Position',[70,195,70,20],'Callback',{@RemoveButton_Callback});
   
   % GUI elements for functions that use all of the current files at once.
   hAllFilesSettings = uicontrol('Style','pushbutton','String','Same bounds/settings','Position',[0,175,140,20],'Callback',{@AllFilesSettingsButton_Callback});
   hAllFilesHist = uicontrol('Style','pushbutton','String','Histogram All','Position',[0,155,140,20],'Callback',{@AllFilesHistButton_Callback});
   hAllFilesThreshold = uicontrol('Style','pushbutton','String','Threshold All','Position',[0,135,140,20],'Callback',{@AllFilesThresholdButton_Callback});
   hAllFilesMean = uicontrol('Style','pushbutton','String','Mean/Stdev All','Position',[0,115,140,20],'Callback',{@AllFilesMeanButton_Callback});
   ...
   % certain value)
   hUpdateBounds = uicontrol('Style','pushbutton','String','Update Bounds','Position',[0,95,140,20],'Callback',{@UpdateBoundsButton_Callback});
   hFilterLabel = uicontrol('Style','text','String','Filter:','Position',[0 65 50 25]);
   hFilterPopup = uicontrol('Style','popupmenu','String',{'No filters yet'},'Position',[50 70 90 20],'Callback',{@ChangeFilterSelection_Callback});
   hThresholdLabel = uicontrol('Style','text','String','Threshold:','Position',[0 50 70 15]);
   hThresholdEntry = uicontrol('Style','edit','String','0','Position',[70 50 70 15]);
   hUpdateThreshold = uicontrol('Style','pushbutton','String','Update Threshold','Position',[0,30,140,20],'Callback',{@UpdateThresholdButton_Callback});
   
   % GUI elements for reporting on # of cells above and below the threshold
   hTotalLabel = uicontrol('Style','text','String','Cells Total:','Position',[0 15 70 15]);
   hAboveLabel = uicontrol('Style','text','String','Cells Above:','Position',[0 0 70 15]);
   hTotalEntry = uicontrol('Style','edit','String','0','Position',[70 15 70 15]);
   hAboveEntry = uicontrol('Style','edit','String','0','Position',[70 0 70 15]);
   ...
   set([f,hAboveLabel,hAllFilesThreshold,hAllFilesSettings,hUpdateBounds,hTotalLabel,hTotalEntry, ...
       hAllFilesHist,hAboveEntry,hFileLabel,hFilterLabel,hAxes,hAxesFilter,hAdd,hRemove,hFileList,hFilterPopup, ...
       hThresholdLabel,hThresholdEntry,hUpdateThreshold,hAllFilesMean],'Units','normalized');
   movegui(f,'center')
   set(f,'Visible','on');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   function UpdateThresholdCounts
       ...
       % negative filter values are ignored as error events.
        [Total, Above] = ArrayOfFCSFiles{get(hFileList,'Value')}.threshold(get(hFilterPopup,'Value'));
   ...
   function PlotHistogram()
       ...
       EventValues = ArrayOfFCSFiles{get(hFileList,'Value')}.getfiltervalues(get(hFilterPopup,'Value'));
       axes(hAxesFilter);
       ...
       set(gca,'XScale','log');
       xlim(XLimits);
       a = ArrayOfFCSFiles{get(hFileList,'Value')}.FilterNames{get(hFilterPopup,'Value')};
       title(strcat(a,' histogram'));
       xlabel(a);
       set(hAxesFilter,'XTick',XTicks)
       ylabel('Normalized count');
       line([ArrayOfFCSFiles{get(hFileList,'Value')}.FilterThresholds(get(hFilterPopup,'Value')) ArrayOfFCSFiles{get(hFileList,'Value')}.FilterThresholds(get(hFilterPopup,'Value'))],[0 1],'Color',[0 0 1]);
   end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   function AllFilesSettingsButton_Callback(source,eventdata)
       ...
       CurrentFilterName = ArrayOfFCSFiles{get(hFileList,'Value')}.FilterNames{get(hFilterPopup,'Value')};
       CurrentThreshold = ArrayOfFCSFiles{get(hFileList,'Value')}.FilterThresholds(get(hFilterPopup,'Value'));
       ...
       for i=1:length(ArrayOfFCSFiles)
            ...
            FilterNumber = find(ismember(ArrayOfFCSFiles{i}.FilterNames,CurrentFilterName));
            % What to do if the relevant filter can't be found
            if isempty(FilterNumber)
                fprintf('The file %s does not have a %s filter; could not set filter threshold.\n',ArrayOfFCSFiles{i}.FileName,CurrentFilterName);
            else
                ArrayOfFCSFiles{i}.FilterThresholds(FilterNumber) = CurrentThreshold;
            end     
       end    
   end
   ...
   function AllFilesHistButton_Callback(source,eventdata)
        ...
        g = figure('Position',[200,200,450,300]); % (The main figure name is 'f'.)
        LegendLabels = {};
        CurrentFilterName = ArrayOfFCSFiles{get(hFileList,'Value')}.FilterNames{get(hFilterPopup,'Value')};
        ...
        for i=1:length(ArrayOfFCSFiles)
            ...
            FilterNumber = find(ismember(ArrayOfFCSFiles{i}.FilterNames,CurrentFilterName));
            if isempty(FilterNumber)
                fprintf('The file %s does not have a %s filter; could not plot on histogram.\n',ArrayOfFCSFiles{i}.FileName,CurrentFilterName);
            else
                % Plot the histogram for this file
                EventValues = ArrayOfFCSFiles{i}.getfiltervalues(FilterNumber);
                BinnedValues = histc(EventValues, HistogramBins);
                BinnedValues = BinnedValues .* 1 / max(BinnedValues);
                plot(HistogramBins,BinnedValues,'Color',hColors(mod(i-1,8)+1,:)); hold on;
   ...
   function AllFilesThresholdButton_Callback(source,eventdata)
       ...
       for i=1:length(ArrayOfFCSFiles)
           FilterNumber = find(ismember(ArrayOfFCSFiles{i}.FilterNames,CurrentFilterName));
           if isempty(FilterNumber)
               fprintf('The file %s does not have a %s filter; could not threshold.\n',ArrayOfFCSFiles{i}.FileName,CurrentFilterName);
           else
               % Threshold the file
   ...
   function AllFilesMeanButton_Callback(source,eventdata)
       ...
       for i=1:length(ArrayOfFCSFiles)
           ...
           if isempty(FilterNumber)
               ...
               [Mean, Stdev] = meanandstdev(ArrayOfFCSFiles{i},FilterNumber);
   ...
   function AddButton_Callback(source,eventdata)
       ...
       [LocalFileNames, LocalFilePath] = uigetfile('*.fcs','Add an FCS file','MultiSelect','on');
       
       % Filenames might be a string, cell array, or numeric
       if isa(LocalFileNames,'cell')
           for i=1:length(LocalFileNames)
               ListOfFiles{length(ListOfFiles) + 1} = fullfile(LocalFilePath, LocalFileNames{i});
               ArrayOfFCSFiles{length(ArrayOfFCSFiles)+1} = FCSFile(fullfile(LocalFilePath, LocalFileNames{i}));
           end
       ...
       elseif isa(LocalFileNames,'numeric')
           ...
           ListOfFiles{length(ListOfFiles) + 1} = fullfile(LocalFilePath, LocalFileNames);
           set(hFileList,'String',ListOfFiles);
           ArrayOfFCSFiles{length(ArrayOfFCSFiles)+1} = FCSFile(fullfile(LocalFilePath, LocalFileNames));
       end
   ...
   function UpdateThresholdButton_Callback(source,eventdata)
       ...
       ArrayOfFCSFiles{get(hFileList,'Value')}.FilterThresholds(get(hFilterPopup,'Value')) = str2num(get(hThresholdEntry,'String'));
       PlotHistogram();
   ...
   function UpdateBoundsButton_Callback(source,eventdata)
       ...
       ArrayOfFCSFiles{get(hFileList,'Value')}.setbounds(BoundPosition(1,:), BoundPosition(2,:));
       cla(hAxes);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   function ChangeFileSelection_Callback(source,eventdata)
       ...
       cla(hAxes);
       ArrayOfFCSFiles{get(hFileList,'Value')}.showfscssc(hAxes);
       set(hFilterPopup,'String', ArrayOfFCSFiles{get(hFileList,'Value')}.FilterNames);
       set(hThresholdEntry,'String',num2str(ArrayOfFCSFiles{get(hFileList,'Value')}.FilterThresholds(get(hFilterPopup,'Value'))));
       cla(hAxesFilter);
       PlotHistogram();
       UpdateThresholdCounts;
   end
